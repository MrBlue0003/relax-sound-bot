"""
monthly_compilation.py — Best Of monthly compilation for Relax Sound Bot.
...
Flow:
  ...
  6. Upload as "Best of [Month Year] | Relax Sound"
...
CLIP_DURATION_SEC = 120        # each relax short is 2 min
TARGET_DURATION_SEC = 30 * 60  # aim for ~30 min compilation
MAX_CLIPS = TARGET_DURATION_SEC // CLIP_DURATION_SEC  # 15
...
def run(uploaded_file: Path, output_base: Path) -> str | None:
    ...
    log_data["uploads"].append({
        ...
        "video_id": video_id,
        "title": f"Best of {month_str} | Relax Sound",
        "url": f"https://www.youtube.com/watch?v={video_id}",
        "is_compilation": True,
